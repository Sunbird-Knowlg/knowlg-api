...
import utils.{ActorNames, ApiId, SchemaConstants}
...
//    uploadRequest.getContext.putAll(Map("identifier" -> identifier, "params" -> UploadParams(fileFormat, validation.map(_.toBoolean))).asJava)
  ...
  def publish(identifier: String) = Action.async { implicit request =>
    val headers = commonHeaders()
    val body = requestBody()
    val content = body.getOrDefault(SchemaConstants.SCHEMA, new java.util.HashMap()).asInstanceOf[java.util.Map[String, Object]]
    content.putAll(headers)
