...
class ObjectActor @Inject() (implicit oec: OntologyEngineContext) extends BaseActor {
      ...
      if (transitionProps.contains(operation)) {
        ...
        DataNode.read(readReq).map(node => {
          if (!fromStatus.contains(node.getMetadata.get("status").toString)) {
            throw new ClientException("ERR_CONTENT_NOT_DRAFT", "Transition not allowed! " + schemaName.capitalize + " Object status should be one of :" + fromStatus.toString())
          }
